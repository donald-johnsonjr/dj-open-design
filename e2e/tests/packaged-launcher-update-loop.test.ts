...
async function writeExtractedWindowsPayload(destinationRoot: string, namespace: string): Promise<void> {
  ...
  await writeFile(join(destinationRoot, "payload", "Kinected Design.exe"), "");
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      channel: "beta",
      entry: { cwd: "payload", executable: "payload/Kinected Design.exe" },
      namespace,
      payloadRoot: "payload",
      platform: "win32",
...
async function writeExtractedMacPayload(destinationRoot: string, namespace: string): Promise<void> {
  const resourcesRoot = join(destinationRoot, "payload", "Kinected Design Beta.app", "Contents", "Resources");
  ...
  await mkdir(join(destinationRoot, "payload", "Kinected Design Beta.app", "Contents", "MacOS"), { recursive: true });
  await writeFile(join(destinationRoot, "payload", "Kinected Design Beta.app", "Contents", "MacOS", "Kinected Design Beta"), "");
...
const platformCases: PlatformCase[] = [
  ...
  },
  {
    ...
    currentVersion: "1.2.3-beta.4",
    expectedResourceRoot: (root, namespace) =>
      join(root, "launcher", "channels", "beta", "namespaces", namespace, "versions", "1.2.3-beta.5", "payload", "Kinected Design Beta.app", "Contents", "Resources", "open-design"),
