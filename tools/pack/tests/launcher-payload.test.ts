...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Kinected Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Kinected Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Kinected Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Kinected Design Beta", "Kinected Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Kinected Design-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Kinected Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Kinected Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Kinected Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Kinected Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Kinected Design.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Kinected Design.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Kinected Design Beta.app",
      channel: "beta",
      entry: {
        cwd: "payload/Kinected Design Beta.app",
        executable: "payload/Kinected Design Beta.app/Contents/MacOS/Kinected Design Beta",
      },
      namespace: "release-beta",
      payloadRoot: "payload",
      platform: "darwin",
    ...
    expect(buildWinLauncherPayloadManifest({
      ...
      entry: {
        cwd: "payload",
        executable: "payload/Kinected Design.exe",
      },
      namespace: "release-beta-win",
      payloadRoot: "payload",
      platform: "win32",
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Kinected Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/Kinected Design Beta.app/Contents/MacOS/Kinected Design Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Kinected Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Kinected Design.exe"));
