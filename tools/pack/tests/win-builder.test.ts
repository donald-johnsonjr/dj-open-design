...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Kinected Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Kinected Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Kinected Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Kinected Design", "Kinected Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Kinected Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Kinected Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Kinected Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Kinected Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Kinected Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Kinected Design.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Kinected Design",
